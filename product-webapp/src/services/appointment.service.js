...
class AppointmentService {
  ...
  appointmentsForPatient(email) {
    return axios.get(
      `http://localhost:8081/api/v1/appointments/patients/${email}`
    );
  ...
  appointmentDetails(appointmentId) {
    ...
    return axios.get(
      `http://localhost:8081/api/v1/appointmentDetails/${appointmentId}`
    );
  }
  appointmentBySpec(spec, activeTab) {
    // return axios.get(`${apiUrl}/api/v1/appointmentDetails/patient/` + appointmentDate)
    return axios.get(
      `http://localhost:8081/api/v1/appointmentBySpec/${spec}/${activeTab}`
    );
  }
  appointmentByDate(appointmentDate, activeTab) {
    // return axios.get(`${apiUrl}/api/v1/appointmentDetails/patient/` + appointmentDate)
    return axios.get(
      `http://localhost:8081/api/v1/appointmentByDate/${appointmentDate}/${activeTab}`
    );
  }
  appointmentByFilter(filter) {
    // return axios.get(`${apiUrl}/api/v1/appointmentDetails/patient/` + appointmentDate)
    return axios.get(
      `http://localhost:8081/api/v1/appointmentFilter?${filter}`
    );
  }
  getSlotsUsingDate(slotDate, email) {
    return axios.get(
      "http://localhost:8080/slotDetails/doctor/" + slotDate + "/" + email
    );
  }
