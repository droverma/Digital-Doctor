...
class ProfileDetailsService {
  ...
  addDoctorProfile(data, userId) {
    ...
    return axios.put("http://localhost:8080/api/v1/doctor/" + userId, data);
  }
  ...
  addPatientProfile(userId, data) {
    return axios.put("http://localhost:8080/api/v1/patient/" + userId, data);
  }

  doctorsList(...patientData) {
    console.log(patientData[0].length);
    console.log(patientData[1].length);
    let apiUrl = `http://localhost:8080/api/v1/doctor?city=${patientData[0]}`;
